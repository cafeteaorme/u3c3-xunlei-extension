...
- 弹窗顶部「迅雷下载」状态区：下载中=琥珀 / 已完成=绿 / 错误=红
  **已完成的条目常驻显示，点击该条目才清除**（悬停有"✕ 点击清除"提示）
- 角标：进行中→橙色数字；有已完成未清除→橙色 ok；逐条点击清除后消减
- 只跟踪本插件发起的任务（infohash 注册表）

## 内测门槛与每日限额（重要）

- 迅雷 NAS 版处于内测期：**非内测名单账号**网页面板被拦（提示输激活码），
  且**每日仅 3 个免费下载任务**（超级会员也不例外——VIP 不限次仅对内测账号生效）
- 限额用尽时 addTask 云端仍受理但任务被静默丢弃，插件会明确提示并指引
- 解决途径（官方提示）：**迅雷内测 QQ 群 772445453 申请内测资格**，或等公测
- 同一迅雷账号最多绑定 3 台 NAS 设备
